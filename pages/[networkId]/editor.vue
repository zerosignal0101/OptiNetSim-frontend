  ...
  <div class="network-editor h-[calc(100vh-150px)] flex flex-col"> <!-- v-loading="isLoading" -->
    ...
    <div class="mb-2 flex items-center space-x-2 p-2 bg-gray-100 dark:bg-gray-800 rounded shadow">
      ...
      <!-- <el-upload
            ...
            :before-upload="handleInsertTopology"
            accept=".json"
    ...
    <div class="flex-grow flex overflow-hidden">
      ...
      <!-- <div
        ...
        <ClientOnly>
            ...
            :nodes="editorStore.graphNodes"
            :edges="editorStore.graphEdges"
            :layouts="layouts"
         ...
         <div v-if="editorStore.editorMode === 'connect' && editorStore.temporaryConnection.source" class="absolute top-0 left-0 p-1 bg-blue-100 text-blue-800 text-xs rounded">
             Connecting from: {{ editorStore.graphNodes[editorStore.temporaryConnection.source]?.name || '...' }} (Click target node)
         </div>
      </div> -->
...
import { ElButton, ElRadioGroup, ElRadioButton, ElDivider, ElTag, ElTooltip, ElIconDocumentChecked, ElIconView, ElIconConnection, ElIconEditPen, ElIconPlus, ElIconVideoPlay, ElMessage, ElLoading, ElUpload } from 'element-plus';
import { VNetworkGraph } from 'v-network-graph';
import type { Configs, EventHandlers, Node, EdgeLayout, Layouts } from 'v-network-graph';
...
const layouts = reactive<Layouts>({
    nodes: {}, // Node positions will be loaded/updated here
});

const graphConfigs = reactive<Configs>({
  view: {
    zoomEnabled: true,
    panEnabled: true,
    ...
    minZoomLevel: 0.1,
    maxZoomLevel: 16,
    layoutHandler: new ForceLayout({ // Example layout algorithm
        positionFixedByDrag: false, // Allow layout to continue after drag
        // Other ForceLayout options...
    }),
  },
  node: {
    selectable: true,
    draggable: (targetNode: Node) => editorStore.editorMode !== 'connect', // Only allow drag in view/edit mode
    normal: {
      radius: 12,
      color: node => node.color || '#4466cc', // Use color from store or default
      strokeWidth: 1,
      strokeColor: '#ffffff'
    },
    hover: {
    ...
    label: {
        visible: true,
        fontSize: 10,
        // LATER: position label based on node count to avoid overlap
    }
  },
  edge: {
    selectable: true,
    ...
    selected: { // Use selection from store, not internal graph selection state
      color: '#0055ee',
      width: 3,
    },
    gap: 5,
...
const graphEventHandlers: EventHandlers = {
    "node:click": ({ node }) => {
    ...
    await editorStore.loadNetwork(id);
     ...
     watch(() => editorStore.elements, (newElements) => {
          if(newElements.size > 0) {
            const initialLayouts: Layouts['nodes'] = {};
